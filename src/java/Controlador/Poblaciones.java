 ...
 * @author Alberto
 */
@WebServlet(urlPatterns = {"/Poblaciones"})
public class Poblaciones extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        ArrayList<String> pueblos = Ulididades.getPoblaciones(rutaFicheros.concat(File.separator).concat("Albacete.txt"));
        request.setAttribute("provincias", provincias);
        request.setAttribute("provincia", "Albacete");
        request.setAttribute("pueblos", pueblos);
        request.getRequestDispatcher("vista/pueblos.jsp").forward(request, response);
    }
